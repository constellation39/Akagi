...
class MajsoulBridge:
    ...
    def input(self, parse_msg: dict) -> dict | None:
        ...
        if (
            ...
            self.mjai_client.launch(self.seat)
            # self.mjai_client.launch(self.seat, self.is_3p)
                ...
                for idx, player in enumerate(parse_msg["data"]["result"]["players"]):
                    if player["seat"] == self.seat:
                        ...
                        game_result_log(
                            ...
                            self.rank,
                            self.score,
                            self.mjai_client.hash(),
                        )
            except:
                pass
            self.mjai_message.append({"type": "end_game"})
            self.react(self.mjai_client)
            self.mjai_client.restart(self.seat)
            return None
        return None

    def react(
        self, mjai_analyser: Analyser, overwrite: str | None = None
    ) -> dict:
        if overwrite is not None:
            # print(f"<- {overwrite}")
            out = mjai_analyser.react(
                str(overwrite)
                .replace("'", '"')
                .replace("True", "true")
                .replace("False", "false")
        ...
        if json_out["type"] == "reach":
            ...
            out = mjai_analyser.react(str(reach).replace("'", '"'))
            json_out = json.loads(out)
        return json_out
