...
from analyser.module.kanachan.constants import (
    _NUM2TILE,
    _TILE2NUM,
    _TILE_OFFSETS,
    _NUM2CHI,
...
from analyser.load_model import get_model_path

from analyser.module.kanachan.hand_calculator import has_yihan, check_kokushi, calculate_shanten
...
class Kanachan:
    def __init__(
            self,
            model_path=get_model_path("model.kanachan"),
    ) -> None:
    ...
    def __respond(self, dapai: Optional[int], candidates: List[int]) -> dict:
        ...
        sparse = [self.__game_state.get_room(), self.__game_state.get_game_style() + 5,
                  self.__game_state.get_player_grade(0) + 7, self.__game_state.get_player_grade(1) + 23,
                  self.__game_state.get_player_grade(2) + 39, self.__game_state.get_player_grade(3) + 55, seat + 71,
                  self.__round_state.get_chang() + 75, self.__round_state.get_index() + 78,
                  self.__round_state.get_num_left_tiles() + 82]
        ...
        sparse = torch.tensor(sparse, device=self.device, dtype=torch.int32).unsqueeze(0)
        ...
        for i in range(4):
            ...
            if player_score is not None:
                ...
                numeric.append(0.0)
        numeric = torch.tensor(numeric, device=self.device, dtype=torch.float32).unsqueeze(0)
        ...
        progression = torch.tensor(
            progression, device=self.device, dtype=torch.int32
        ).unsqueeze(0)
        ...
        candidates_ = torch.tensor(
            candidates_, device=self.device, dtype=torch.int32
        ).unsqueeze(0)
        ...
        with torch.no_grad():
            ...
            if len(progression) == 3:
                ...
                raise ValueError("progression.size() should be 3 or 4")
        ...
        mask_unicode_4p_dict = {"1m": 0, "2m": 0, "3m": 0, "4m": 0, "5m": 0, "6m": 0, "7m": 0, "8m": 0, "9m": 0,
                                "1p": 0,
                                "2p": 0, "3p": 0, "4p": 0, "5p": 0, "6p": 0, "7p": 0, "8p": 0, "9p": 0, "1s": 0,
                                "2s": 0,
                                "3s": 0, "4s": 0, "5s": 0, "6s": 0, "7s": 0, "8s": 0, "9s": 0, "E": 0, "S": 0, "W": 0,
                                "N": 0, "P": 0, "F": 0, "C": 0, "5mr": 0, "5pr": 0, "5sr": 0, "reach": 0, "chi_low": 0,
                                "chi_mid": 0, "chi_high": 0, "pon": 0, "kan_select": 0, "hora": 0, "ryukyoku": 0,
                                "none": 0,
                                }

        def fill_meta(out: dict) -> dict:
            bit_mask = 0
            for i, key in enumerate(mask_unicode_4p_list):
                value = mask_unicode_4p_dict[key]
                bit_mask |= (value << i)

            meta = {
                "q_values": decode.tolist(),
                "eval_time_ns": time.time() - start_time,
                "mask_bits": bit_mask
            }
        ...
        if 0 <= decision <= 147:
            ...
            mask_unicode_4p_dict[tile] = 1

            if liqi:
                mask_unicode_4p_dict["reach"] = 1
                return fill_meta({"type": "reach", "actor": seat})
            return fill_meta({"type": "dahai", "actor": seat, "pai": tile, "tsumogiri": moqi})

        if 148 <= decision <= 181:
            angang = _NUM2ANGANG[decision - 148]
            mask_unicode_4p_dict["kan_select"] = 1
            return fill_meta({"type": "ankan", "actor": seat, "consumed": angang})

        if 182 <= decision <= 218:
            tile, consumed = _NUM2JIAGANG[decision - 182]
            mask_unicode_4p_dict["kan_select"] = 1
            return fill_meta({"type": "kakan", "actor": seat, "pai": tile, "consumed": consumed})
        ...
        if decision == 219:
            ...
            mask_unicode_4p_dict["hora"] = 1
            return fill_meta({"type": "hora", "actor": seat, "target": seat, "pai": hupai})

        if decision == 220:
            mask_unicode_4p_dict["ryukyoku"] = 1
            return fill_meta({"type": "ryukyoku"})
        ...
        if decision == 221:
            ...
            for i in (543, 544, 545):
                if i in candidates:
                    ...
                    break
            mask_unicode_4p_dict["none"] = 1
            return fill_meta({"type": "none"})

        if 222 <= decision <= 311:
            tile, consumed = _NUM2CHI[decision - 222]
            mask_unicode_4p_dict["chi_low"] = 1
            return fill_meta({
                "type": "chi",
                "actor": seat,
                "target": (seat + 3) % 4,
        ...
        if 543 <= decision <= 545:
            ...
            hupai = _NUM2TILE[hupai]
            mask_unicode_4p_dict["hora"] = 1
            return fill_meta({"type": "hora", "actor": seat, "target": target, "pai": hupai})
