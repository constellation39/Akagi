...
class LiqiServer:
    ...
    def get_messages(self, flow_id):
        try:
            ...
            idx = 0
        if (
            (flow_id not in activated_flows)
            or (len(messages_dict[flow_id]) == 0)
            or (self.message_idx[flow_id] >= len(messages_dict[flow_id]))
        ):
            return None
...
class PlaywrightController:
    def __init__(self, width, height, mitm_port):
        ...
        self.browser = self.chromium.launch_persistent_context(
            ...
            # proxy={"server": f"http://localhost:{10808}"},
        ...
        self.scale = width / 16
        self.tiles_location_scaled = [
            [x * self.scale, y * self.scale] for x, y in LOCATION["tiles"]
        ]
...
if __name__ == "__main__":
    ...
    if enable_playwright:
        playwright_controller = PlaywrightController(
            playwright_width, playwright_height, mitm_port
        )
