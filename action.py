...
class Action:
    ...
    def click_chiponkan(
        self, mjai_msg: dict | None, tehai: list[str], tsumohai: str | None
    ):
                ...
                for operation in self.latest_operation_list:
                    if operation["type"] == 2:
                        ...
                        for idx, combination in enumerate(operation["combination"]):
                            consumed_pais_liqi = [
                                MS_TILE_2_MJAI_TILE[pai]
                                for pai in combination.split("|")
                            ]
    ...
    def get_pai_coord(self, idx: int, tehais: list[str]):
        ...
        if idx == 13:
            pai_cord = (
                LOCATION["tiles"][tehai_count][0] + LOCATION["tsumo_space"],
                LOCATION["tiles"][tehai_count][1],
            )
        else:
